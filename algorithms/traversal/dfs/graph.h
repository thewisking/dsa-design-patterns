...
class Graph {
  private:
    ...
    bool dfs_helper(std::size_t current, std::size_t destination, std::vector<bool> &visited, int &traversalCount) {
        // std::cout << current << "   ";   :>
        ...
        for (const Node &neighbour : m_list[current]) {
            ...
            if (!visited[value]) {
                ++traversalCount;
                if (dfs_helper(value, destination, visited, traversalCount)) {
                    return true;
                }
            }
        }
  ...
  public:
    ...
    int dfs(int source_index, int destination_index) {
        ...
        bool found = dfs_helper(source, destination, visited, traversalCount);
        if (!found) {
            return -1;
        }
